...
def create_supervisor_agent(query):
    ...
    #mcp_client = MCPClient(streamablehttp_client(MCP_SERVER_URL,headers={"Authorization": f"Bearer {token}"}))
...
def invoke(payload):
    ...
    try:
        print(f"Payload123: {payload}")
        user_message = payload.get("prompt", "what is the credit score for customer id 1234??")
